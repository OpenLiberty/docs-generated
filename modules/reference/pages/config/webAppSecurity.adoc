...
|+allowInMemoryIdentityStores+

|boolean

|+false+

|+When explicitly set to true and an in-memory identity store is defined within an application, the in-memory identity store is used during the authentication process.+
...
|+A unique configuration ID.+
|===
[#+loggedOutCookieCache/cacheManager/cachingProvider/commonLibrary/path+]*loggedOutCookieCache > cacheManager > cachingProvider > commonLibrary > path*
...
|+A unique configuration ID.+
|===
[#+loggedOutCookieCache/cacheManager/cachingProvider/jCacheLibrary/path+]*loggedOutCookieCache > cacheManager > cachingProvider > jCacheLibrary > path*
