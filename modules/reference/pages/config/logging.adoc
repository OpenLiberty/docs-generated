...
|+Specifies the maximum number of log messages permitted within a 5-minute period before throttling begins. When the limit is exceeded, additional messages are suppressed until the count is within the threshold. The default value is 1000. You can specify a positive integer or set the value to 0 to disable throttling.+
...
|+Tracks log entries that match either the messageID or the full message content. Specify a string to configure this attribute. The only valid values are message and messageID.+ +
*+message+* +
+Specifies throttling of log entries based on the message content.+ +
*+messageID+* +
+Specifies throttling of log entries based on the messageID.+
