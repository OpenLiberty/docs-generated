...
|+A unique configuration ID.+
|===
[#+cloudant/library/path+]*cloudant > library > path*
...
|+Two mutually exclusive modes: (1) Specify a custom list of ciphers separated by spaces, or (2) Specify filter criteria to add (pass:[+]) or remove (-) cipher suites from the effective JDK cipher suites. In filter mode, prefix cipher suite names with pass:[+] to add them or - to remove them. You can use the * wildcard with - to remove multiple cipher suites that share a prefix (for example, -TLS_RSA_*); place the wildcard only at the end of the list item. Do not use the wildcard when adding cipher suites. The underlying JRE determines which cipher suites are supported.+
...
|+Specifies whether host name verification for outbound connections using a specific SSL configuration is enabled. If set to true, then all outbound SSL connections that use the specified SSL configuration undergo verification of the target server host name against that server's certificate.+
|===
