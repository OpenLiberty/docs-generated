...
*+loose+* +
+Maximum concurrency is loosely enforced. Tasks are allowed to run on the task submitter's thread without counting against maximum concurrency. Loose is the default when the concurrency policy uses platform threads.+ +
*+strict+* +
+Maximum concurrency is strictly enforced. Tasks that run on the task submitter's thread count towards maximum concurrency. This policy does not allow tasks to run on the task submitter's thread when they are already at maximum concurrency. Strict is the default when the concurrency policy uses virtual threads.+
...
|+Requests the use of virtual threads for tasks that do not run inline. Java 21 or higher is a prerequisite of configuring this value to true.+
|===
