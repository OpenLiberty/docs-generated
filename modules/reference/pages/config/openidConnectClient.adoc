...
|+Specifies a list of signature algorithms that are allowed when the software is validating ID tokens. The signature algorithm from the JWT header must be in this allowed list for the token to be accepted. This attribute applies only when the signatureAlgorithm attribute is set to FROM_HEADER.+ +
*+ES256+* +
...
+Use the ES512 signature algorithm to sign and verify tokens+ +
*+FROM_HEADER+* +
+Use the signature algorithm from the JWT header to sign and verify tokens.+ +
*+HS256+* +
+Use the HS256 signature algorithm to sign and verify tokens+ +
*+HS384+* +
+Use the HS384 signature algorithm to sign and verify tokens+ +
...
+Specifies the configuration for serving OAuth 2.0 protected resource metadata (RFC 9728). When configured, the OpenID Connect client can provide metadata to direct clients on where to authenticate for protected resources.+
...
|+Specifies a comma-separated list of scopes to publish in the protected resource metadata. Clients can use the list to discover which scopes are available for this protected resource.+
...
|+Specifies a reference to a JSON Web Token (JWT) builder configuration. Use the builder to create JSON web tokens for the protected resource metadata endpoint.+
|===
[#+protectedResourceMetadata/jwtBuilder+]*protectedResourceMetadata > jwtBuilder*
...
|+Specifies the key management algorithm that is used to encrypt or derive the Content Encryption Key of a JWE.+ +
*+ECDH-ES+* +
+Use the ECDH-ES algorithm to derive the Content Encryption Key of a JWE.+ +
*+RSA-OAEP+* +
...
|+A keystore that contains the key management key that is used to encrypt or derive the Content Encryption Key of a JWE.+
|===
