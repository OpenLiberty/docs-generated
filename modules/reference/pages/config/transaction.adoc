...
|+correctUOWScopeCallbacks+

|boolean

|+false+

|+Specifies whether UOWScopeCallback methods should be called for UOWScope END events.+
...
|+enableUserTransactionAsSpecified+

|boolean

|+false+

|+Specifies whether UserTransaction methods should be enabled in NOT_SUPPORTED and NEVER @Transactional annotated methods.+
...
|+Specifies whether methods annotated with @Transactional can throw undeclared checked exceptions.+
...
|+A unique configuration ID.+
|===
[#+dataSource/jdbcDriver/library/path+]*dataSource > jdbcDriver > library > path*
...
|+JDBC driver property: serverName. This property previously defaulted to localhost, but that default was removed to support valid use cases where no serverName property is provided. Examples include when the DB2 driver uses type java.sql.Driver with a URL, or when a client catalog is used with the driverType property set to 2. In all other cases, if this property is unset, a default value of localhost is added programmatically. This property overrides the server name that is specified in the URL and in the client catalog if either was configured.+
...
|+JDBC driver property: databaseName. When the PostgreSQL driver uses type java.sql.Driver, and both a URL and the databaseName property are configured, the PostgreSQL driver ignores the databaseName property. When the PostgreSQL driver uses type javax.sql.DataSource, and both a URL and the databaseName property are configured, the PostgreSQL driver ignores the database name that is embedded in the URL.+
...
|+JDBC driver property: portNumber. When the PostgreSQL driver uses type java.sql.Driver, and both a URL and the portNumber property are configured, the PostgreSQL driver ignores the portNumber property. When the PostgreSQL driver uses type javax.sql.DataSource, and both a URL and the portNumber property are configured, the PostgreSQL driver ignores the port number that is embedded in the URL.+
...
|+JDBC driver property: serverName. When the PostgreSQL driver uses type java.sql.Driver, and both a URL and the serverName property are configured, the PostgreSQL driver ignores the serverName property. When the PostgreSQL driver uses type javax.sql.DataSource, and both a URL and the serverName property are configured, the PostgreSQL driver ignores the server name that is embedded in the URL.+
