...
|+If you want a Content-Security-Policy HTTP header to be included in the initial login request that is sent to your IdP, set the contentSecurityPolicy attribute to the value that you want to use for the Content-Security-Policy HTTP header. If your Content-Security-Policy value requires a nonce, you can use the "%NONCE%" keyword to indicate where the nonce should be placed in the text, as shown in the following example: "script-src 'self' 'nonce-%NONCE%' ; object-src 'self'; frame-src 'self'"+
...
|+Indicates the minimum signature algorithm that is required by this service provider.+ +
*+ECDSAwithSHA256+* +
