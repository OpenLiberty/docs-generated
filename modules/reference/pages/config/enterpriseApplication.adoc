...
|+annotationScanLibrary+

|string

|

|+Configure which libraries are going to be scanned for Web Application annotations in a comma seperated list. Valid options are earLibraries, manifestClassPath, or all.+
...
|+A unique configuration ID.+
|===
[#+classloader/commonLibrary/path+]*classloader > commonLibrary > path*
...
|===
[#+classloader/overrideLibrary+]*classloader > overrideLibrary*
...
|+Name of shared library for administrators+
|===
[#+classloader/overrideLibrary/file+]*classloader > overrideLibrary > file*
...
|+The path to a file can be a fully qualified path or a relative path. Relative paths are relative to the value of the ${server.config.dir} property.+
|===
[#+classloader/overrideLibrary/fileset+]*classloader > overrideLibrary > fileset*
...
|+The scanning interval to determine whether files are added or removed from the fileset. The individual files are not scanned. The suffix for the interval of time is h-hour, m-minute, s-second, and ms-millisecond, for example, 2ms or 5s. The scanning interval is disabled by default and is disabled manually by setting the scan interval, scanInterval, to 0. Specify a positive integer followed by a unit of time, which can be hours (h), minutes (m), seconds (s), or milliseconds (ms). For example, specify 500 milliseconds as 500ms. You can include multiple values in a single entry. For example, 1s500ms is equivalent to 1.5 seconds.+
|===
[#+classloader/overrideLibrary/folder+]*classloader > overrideLibrary > folder*
...
|+A unique configuration ID.+
|===
[#+classloader/overrideLibrary/path+]*classloader > overrideLibrary > path*
...
|+A unique configuration ID.+
|===
[#+classloader/privateLibrary/path+]*classloader > privateLibrary > path*
...
|+A unique configuration ID.+
|===
[#+startAfter/classloader/commonLibrary/path+]*startAfter > classloader > commonLibrary > path*
...
|+The path can be a fully qualified path or a relative path. Relative paths are relative to the value of  the ${server.config.dir} property.+
|===
[#+startAfter/classloader/overrideLibrary+]*startAfter > classloader > overrideLibrary*
...
|+Name of shared library for administrators+
|===
[#+startAfter/classloader/overrideLibrary/file+]*startAfter > classloader > overrideLibrary > file*
...
|+The path to a file can be a fully qualified path or a relative path. Relative paths are relative to the value of the ${server.config.dir} property.+
|===
[#+startAfter/classloader/overrideLibrary/fileset+]*startAfter > classloader > overrideLibrary > fileset*
...
|+The scanning interval to determine whether files are added or removed from the fileset. The individual files are not scanned. The suffix for the interval of time is h-hour, m-minute, s-second, and ms-millisecond, for example, 2ms or 5s. The scanning interval is disabled by default and is disabled manually by setting the scan interval, scanInterval, to 0. Specify a positive integer followed by a unit of time, which can be hours (h), minutes (m), seconds (s), or milliseconds (ms). For example, specify 500 milliseconds as 500ms. You can include multiple values in a single entry. For example, 1s500ms is equivalent to 1.5 seconds.+
|===
[#+startAfter/classloader/overrideLibrary/folder+]*startAfter > classloader > overrideLibrary > folder*
...
|+A unique configuration ID.+
|===
[#+startAfter/classloader/overrideLibrary/path+]*startAfter > classloader > overrideLibrary > path*
...
|+A unique configuration ID.+
|===
[#+startAfter/classloader/privateLibrary/path+]*startAfter > classloader > privateLibrary > path*
...
|+Stateless mode disables MCP session tracking by the server. This mode removes the need for session affinity in a clustered environment. Otherwise, the same server handles requests from the same MCP session. Because no session data is stored on the servers, any server instance can handle any request, which simplifies load balancing and horizontal scaling. However, the stateless mode also disables MCP features that rely on session tracking or linking separate HTTP requests together, such as canceling a tool call.+
|===
[#+startAfter/mcp/info+]*startAfter > mcp > info*
