...
|+Specifies a list of signature algorithms that are allowed when the software is validating JWTs. The signature algorithm from the JWT header must be in this allowed list for the token to be accepted. This attribute applies only when the signatureAlgorithm attribute is set to FROM_HEADER.+ +
*+ES256+* +
